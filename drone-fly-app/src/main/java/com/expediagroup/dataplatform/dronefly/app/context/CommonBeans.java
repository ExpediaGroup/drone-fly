...
public class CommonBeans {
  ...
  public ListenerCatalog listenerCatalog(HiveConf conf) throws MetaException {
    ListenerCatalog listenerCatalog =
        new ListenerCatalogFactory(conf).newInstance(confListenerList);
    List<MetaStoreEventListener> listenerList = listenerCatalog.getListeners();
    String listeners =
        listenerList.stream().map(x -> x.getClass().getName()).collect(Collectors.joining(", "));
  ...
  public MessageReaderAdapter messageReaderAdapter() {
    ...
    KafkaMessageReader delegate =
        KafkaMessageReaderBuilder.builder(bootstrapServers, topicName, instanceName)
