 ...
 *
 * <p>Based on {@code org.apache.hadoop.hive.metastore.MetaStoreUtils} from hive-metastore 2.3.7:
 *
 * <p>https://github.com/apache/hive/blob/rel/release-2.3.7/metastore/src/java/org/apache/hadoop/hive/metastore/MetaStoreUtils.java#L1642
 *
  ...
  private <T> List<T> getMetaStoreListeners(Class<T> clazz, HiveConf conf, String listenerImplList)
    ...
    for (String listenerImpl : listenerImpls) {
      try {
        T listener =
            (T)
                Class.forName(listenerImpl.trim(), true, JavaUtils.getClassLoader())
                    .getConstructor(Configuration.class)
                    .newInstance(conf);
        listeners.add(listener);
      } catch (Exception e) {
        throw new DroneFlyException(
            "Failed to instantiate listener named: " + listenerImpl + ", reason: ", e);
      }
    }
